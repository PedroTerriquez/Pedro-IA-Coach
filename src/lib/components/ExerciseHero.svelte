  ...
  <div class="hero" class:logged-hero={loggedToday} style="border-color:{loggedToday ? accent : 'rgba(255,255,255,0.06)'};box-shadow:{loggedToday ? `0 0 0 4px ${accent}1a,0 8px 32px ${accent}22` : 'none'}">
    ...
    <div class="hero-bottom-row">
      ...
      <div class="hero-search-btns">
        <button class="search-btn hero-google-btn" aria-label="Buscar en Google" onclick={() => { location.href = `https://www.google.com/search?tbm=video&q=${encodeURIComponent(displayName + ' exercise')}` }}>
        ...
        <button class="search-btn hero-tiktok-btn" aria-label="Buscar en TikTok" onclick={() => { location.href = `https://www.tiktok.com/search?q=${encodeURIComponent(displayName + ' exercise')}` }}>
