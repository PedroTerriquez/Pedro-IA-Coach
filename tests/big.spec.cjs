...
test.describe('Admin: revisar y corregir media del diccionario', () => {
  test('abre admin, busca candidato y guarda un cambio de image (y lo revierte)', async ({ page }) => {
    test.setTimeout(120000)
    const DEV_BASE = 'http://localhost:5174/Pedro-IA-Coach'
    const SETTINGS = {
      id: 'settings',
    ...
    await page.goto(`${DEV_BASE}/you`)
    await seedIndexedDB(page, { exercises: [], settings: SETTINGS })
    await page.reload()
    await page.waitForTimeout(400)

    // Seed catalogs so the picker search is deterministic (no GitHub network call)
    await page.evaluate(() => {
    ...
    // First entry card and its current image URL
    ...
    // Letter pagination: filtering by P narrows the list to Press Banca + peers
    const allCount = (await page.locator('.count').textContent())?.trim()
    await page.locator('.letters button.chip', { hasText: 'P' }).click()
    await expect(page.locator('.count')).toHaveText(/^\d+ ejercicios$/)
    await expect(card).toBeVisible()
    const cards = await page.locator('[data-component="AdminCard"]').count()
    expect(cards).toBeLessThan(166)
    expect((await page.locator('.count').textContent())?.trim()).not.toBe(allCount)
    await page.locator('.letters button.chip', { hasText: 'A' }).click()
    await expect(page.locator('.count')).toHaveText(allCount)

    // Open image picker, search the seeded candidate, pick it → queue a draft
    ...
    // Save → plugin rewrites the dictionary file → HMR updates the preview
    ...
    // Revert so the source file stays byte-identical
    await card.locator(`#edit-img-${entryId}`).click()
